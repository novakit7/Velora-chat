...
import { FiArrowLeft, FiCheck, FiClock, FiUserPlus, FiX } from "react-icons/fi";
...
} from "../../socket/Listeners/friend.listener";
...
export default function FriendSection() {
  ...
  const handleRequestAccepted = ({ requestId }) => {
    ...
    notify.success("Your friend request was accepted.");
  };

  const handleRequestRejected = ({ requestId }) => {
    console.log("Socket requestId:", requestId);
    setSentRequests((prev) =>
      prev.filter((request) => request._id !== requestId),
    );

    notify.error("Your friend request was rejected.");
  };
  ...
  const handleRequestCancelled = ({ requestId }) => {
    ...
    notify("Friend request was cancelled.");
  };
  ...
  const handleNewFriendRequest = (request) => {
    ...
    notify.success(`${request.sender.username} sent you a friend request`);
  };

  // Register socket listeners
  useEffect(() => {
    registerFriendListeners({
      onFriendReceived: handleNewFriendRequest,
          ...
          <section className="flex flex-col">
            ...
            {incomingRequests.length === 0 ? (
              <div className="rounded-xl border border-slate-800 bg-slate-800/40 py-12 text-center">
                ...
                <p className="text-slate-400">No incoming friend requests.</p>
              </div>
            ) : (
